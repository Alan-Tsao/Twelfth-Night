...
function getCastInfo(name){
  return (window.allCasts||[]).find(c=>c.name===name)||{name,shortDesc:"",desc:"",tags:[]};
}
...
function renderCastOptionsFromSheet(date,box,pend,hint){
  ...
  const availableRows=rowsForDate.filter(r=>r.status==="available" && rowCoversRequestedTime(r));
  const pendingRows=rowsForDate.filter(r=>r.status==="pending" && rowCoversRequestedTime(r));
  ...
  if(pend){
    if(pendingRows.length){
      ...
      pend.innerHTML=`詢問制公關：${pendingRows.map(row=>`${escapeHtml(row.cast)}（${escapeHtml(scheduleTimeText(row))}${row.note?`｜${escapeHtml(row.note)}`:""}）`).join("、")}。這些公關目前不會列入可勾選名單，可於備註中詢問接待。`;
    }else{
      pend.classList.remove("show");
      pend.textContent="";
    }
